...
def medicine_info(master, func, handle):
    ...
    identry_id = tk.Entry(search_line, textvariable=medicine_id, width=25, bd=0, font=myFont, bg='lightsteelblue', name='entry_medicine_id')
    ...
    identry_name = tk.Entry(search_line0, textvariable=medicine_name, width=25, bd=0, font=myFont, bg='lightsteelblue', name='entry_medicine_name')
    ...
    tk.Button(search_line0, text='查询所有', font=('song ti', 13), bd=2, command=find_all, fg="#F6FDFE", bg='#4ABD97') \
        .pack(side='left', padx=(0, 15))
    tk.Button(search_line0, text='添加', font=('song ti', 13), bd=2, command=add_medicine, fg="#F6FDFE", bg='#4ABD97', name='button_add') \
        .pack(side='left', padx=(0, 15))
    tk.Button(search_line0, text='删除', font=('song ti', 13), bd=2, command=del_medicine, fg="#F6FDFE", bg='#4ABD97', name='button_delete') \
        .pack(side='left')
    ...
    tk.Entry(change_line, textvariable=change_id, width=5, bd=0, font=myFont, bg='lightsteelblue', name='entry_change_id')\
        .pack(side='left', padx=(0, 5))
    tk.Label(change_line, text='要更改的条目:', bg='white', font=('song ti', 13)).pack(side='left', padx=(10, 0))
    del_tag = ttk.Combobox(change_line, textvariable=change_item, values=['', '药名'], width=10, name='combobox_change_item')
    del_tag.current(0)
    del_tag.pack(side='left', padx=(10, 0))
    tk.Label(change_line, text='更改为:', bg='white', font=('song ti', 13)).pack(side='left', padx=(10, 0))
    tk.Entry(change_line, textvariable=change_info, width=15, bd=0, font=myFont, bg='lightsteelblue', name='entry_change_info')\
        .pack(side='left', padx=(0, 5))
    tk.Button(change_line, text='更改', font=('song ti', 13), bd=2, command=change_medicine, fg="#F6FDFE", bg='#4ABD97', name='button_change') \
        .pack(side='left')
...
if __name__ == '__main__':
    root = tk.Tk()
    medicine_info(root, root.quit, handler())
    root.mainloop()